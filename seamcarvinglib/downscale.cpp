...
graph insert_h_seam(graph &g)
{
	...
	New.pixelarray.push_back(pixel(New.pixelarray[cur].val[0],New.pixelarray[cur].val[1],New.pixelarray[cur].val[2] ));
	New.pixelarray_size ++;
	int Top = New.pixelarray[cur].top, Bottom = New.pixelarray[cur].bottom;
	...
	for(i=m-1;i>=0;--i)
	{
		...
		if(pos[i]==pos[i-1]){
			...
			Top = New.pixelarray[cur].top; Bottom = New.pixelarray[cur].bottom;
			New.pixelarray.push_back(pixel(New.pixelarray[cur].val[0],New.pixelarray[cur].val[1],New.pixelarray[cur].val[2]));
			New.pixelarray_size ++;
...

{
	...
	for(i=n-1;i>=0;--i)
	{
		...
		if(pos[i]==pos[i-1]){
			...
			cur = New.pixelarray[cur].top;
			Left = New.pixelarray[cur].left; Right = New.pixelarray[cur].right;
		//create new pixel at index 
...
{
	upscale(image, (int)(image.size().height*(r_height - 1)), (int)(image.size().width*(r_width -1)));
}
int main()
{
	// cout<<"starting!"<<endl;
	...
	// waitKey(0);  
	graph g(image);
upscale(image, 1.0, 1.2);					g.convertgraphtoimage();;
	return 0;
